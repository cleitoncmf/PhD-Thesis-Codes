function Y = FN_RF_Yac_i_multi_dq_srf(MMC_structure,Gidq,G5,G7)
...
psi57 = psi5+psi7;
H_psi57 = freqresp(psi57,w);

%Didq_srf
Didq_srf = ((L+2*Lf)/Vdc0).*W;

%Gchar_srf
Gchar_srf =I + (4*Vdc0*Ceq.*sdq + (2*SN/(3*Vdc0)).*I)*(Gidq+psi57) + 4*Ceq*(Z+2*Zf).*sdq -(2*SN/(3*Vdc0)).*Didq_srf;
