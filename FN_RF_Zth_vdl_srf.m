...
function Y = FN_RF_Zth_vdl_srf(MMC_structure,Gidq,Gvdq)
...
%Gcharv_srf
Gcharv_srf = Gnorton_srf*Gvdq + Yac_srf;

%Gcharv_srf_inv
Gcharv_srf_inv = minreal(inv(Gcharv_srf),0.02);
H_Gcharv_srf_inv = freqresp(Gcharv_srf_inv,w);
...
%Zth3_srf = @(Gidq,Gvdq) charV2_srf(Gidq,Gvdq)\Gcharvinv3_srf(Gidq,Gvdq);
H_Zth_dl_srf = H_charV_srf_inv.*H_Gcharv_srf_inv;
...
for k=1:length(w)
    Mag_dd_cplx(k) = H_Zth_dl_srf(1,1,k);
    Mag_dq_cplx(k) = H_Zth_dl_srf(1,2,k);
    Mag_qd_cplx(k) = H_Zth_dl_srf(2,1,k);
    Mag_qq_cplx(k) = H_Zth_dl_srf(2,2,k);
    Mag_0_cplx(k) = H_Zth_dl_srf(3,3,k);
end
